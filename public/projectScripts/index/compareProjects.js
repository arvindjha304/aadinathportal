...
function addRemoveCompare(project_id,is_menu=0){
    ...
    if(project_id!==''){
        if ($.inArray(project_id, CompareProjectsArr) !== -1) {
               $('#addCompareDiv_'+project_id).html('<a  data-toggle="tooltip" title="Maximum 3 Projects" onclick=\'addRemoveCompare('+project_id+')\'><span class="glyphicon glyphicon-unchecked" aria-hidden="true" ></span> Add to Compare</a>');
            ...
            if(CompareProjectsArr.length !=3){
                $('#addCompareDiv_'+project_id).html('<a onclick=\'addRemoveCompare('+project_id+')\'><span class="glyphicon glyphicon-check" aria-hidden="true" ></span> Remove from Compare</a>');
